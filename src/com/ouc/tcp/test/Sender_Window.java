...
public class Sender_Window {
    ...
    private int RENO_FLAG = 1; // 修改RENO_FLAG可以切换Tahoe版本和Reno版本

    public int cwnd = 1;
    private volatile int ssthresh = 16;
    private int CongestionAvoidanceCount = 0;  // 进入拥塞避免状态时收到的ACK数
   ...
   /*构造函数*/
    ...
    public void putPacket(TCP_PACKET packet) {
        ...
        if (packets.isEmpty()) {
            // 窗口左沿则开始计时器
            timer = new Timer();
            timer.schedule(new TCP_Retry(client, this), 3000, 3000);
        }
    ...
    public void receiveACK(int currentSequence) {
        ...
        if (currentSequence == lastACKSequence) {
            ...
            if (cwnd < ssthresh) {
                ...
                if (CongestionAvoidanceCount == cwnd) {
